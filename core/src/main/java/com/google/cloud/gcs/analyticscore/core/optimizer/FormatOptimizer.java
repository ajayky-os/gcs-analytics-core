...
public interface FormatOptimizer {
  ...
  void onOpen(GcsItemId itemId, AnalyticsCacheManager cacheManager);

  /** Invoked when the channel is opened with full file metadata. */
  default void onOpen(GcsFileInfo fileInfo, AnalyticsCacheManager cacheManager) {
   ...
   */
  int read(long position, ByteBuffer dst, VectoredSeekableByteChannel source) throws IOException;
